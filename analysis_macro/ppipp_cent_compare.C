...
TF1* optimizeAndFitProton(TH1D* hist, double p_min, double p_max)
{
    ...
    if (isLowPt) { finalFit->SetParLimits(0, peak_height * 0.3, peak_height * 0.92); } 
    else { finalFit->SetParLimits(0, peak_height * 0.1, peak_height * 1.5); }
    finalFit->SetParLimits(1, -0.1, 0.1); finalFit->SetParLimits(2, 0.8, 1.15);
    if (isLowPt) { finalFit->SetParLimits(3, peak_height * 0.30, peak_height * 1.0); } 
    else { finalFit->SetParLimits(3, peak_height * 0.1, peak_height * 1.5); }
    finalFit->SetParLimits(4, 0.7, 1.6); finalFit->SetParLimits(5, 0.8, 1.2); 
    finalFit->SetParLimits(6, 0.0, peak_height * 1.2); finalFit->SetParLimits(7, 3.0, 6.0); finalFit->SetParLimits(8, 0.5, 2.0);
    finalFit->SetParLimits(9, 0.0, peak_height * 0.5); finalFit->SetParLimits(10, 7.0, 12.0); finalFit->SetParLimits(11, 0.6, 2.0);
...
TH1D* GetRatioForCentrality(TString dataFile, TString mcFile, double centMin, double centMax, int color, int markerStyle, TString label) {
    ...
    // 注意: ここではMCの統計を確保するため、MC側は0-100% (Inclusive)を使用する設定にしています。
    // Dataと同じCentralityのMCを使用したい場合は、下の引数を centMin, centMax に変更してください。
    double mcCentMin = 0.0;
    double mcCentMax = 100.0;
    ...
    hRatio->SetDirectory(0); // Memory management
...
void CompareCentralities() {
    ...
    const TString dataFile = "AnalysisResults-83.root";
    ...
    // --- Plotting ---
    TCanvas *c1 = new TCanvas("cComp", "Ratio Comparison", 800, 600);
    gPad->SetLeftMargin(0.15);
    gPad->SetBottomMargin(0.12);
    gPad->SetGridy();

    // Axis settings (using 0-30 histogram as frame)
    hRatio0_30->SetTitle("");
    ...
    hRatio0_30->SetMaximum(1.2); // 少し余裕を持たせる
    ...
    c1->SaveAs("Ratio_Comparison_0-30_vs_60-100.pdf");
}